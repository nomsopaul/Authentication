      ...
      .then (profile => {
        if (profile) {
          Profile.findOneAndUpdate (
            ...
            .catch (err => console.log ('problem updating user profile' + err));
        } else {
...
router.get ('/:username', (req, res) => {
  Profile.findOne ({username: req.params.username})
    .populate ('user', ['name', 'profilepic', 'email', 'password'])
    .then (profile => {
      if (!profile) {
    ...
    .catch (err => console.log ('Error in username provided ' + err));
});
//@ type get
    ...
    Profile.findOneAndRemove ({user: req.user.id})
      .then (() => {
        Person.findOneAndRemove ({_id: req.user.id})
          .then (() => res.json ({success: 'delete operation was successful'}))
          .catch (err => console.log (err));
      })
      .catch (err => console.log (err));
  }
...
router.post (
  ...
  (req, res) => {
    Profile.findOne ({user: req.user.id})
      .then (profile => {
        res.json ({success: 'Successsfully Updated'});
        const workrole = {
          role: req.body.role,
...
router.delete (
  ...
  (req, res) => {
    Profile.findOne ({user: req.user.id})
      .then (profile => {
        res.json ({success: 'Profile found'});
